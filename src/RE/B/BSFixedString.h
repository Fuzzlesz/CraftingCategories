...
namespace RE
{
	namespace detail
	{
		...
		public:
			...
			inline BSFixedString(const BSFixedString& a_rhs) : _data(a_rhs._data)
			{
				try_acquire();
			}

			inline BSFixedString(BSFixedString&& a_rhs) : _data(a_rhs._data)
			{
				a_rhs._data = nullptr;
			}
			...
			template <
				...
				std::enable_if_t<
					std::conjunction_v<
						...
						std::negation<std::is_convertible<const T&, const_pointer>>>,
					int> = 0>
			...
			[[nodiscard]] constexpr operator std::basic_string_view<value_type>() const
			{
				return { c_str(), length() };
			}
			...
			[[nodiscard]] inline friend bool operator==(
				const BSFixedString& a_lhs,
				const BSFixedString& a_rhs) noexcept
			{
			...
			[[nodiscard]] inline friend bool operator==(
				std::basic_string_view<value_type> a_lhs,
			...
			[[nodiscard]] inline friend bool operator==(
				...
				const_pointer a_rhs)
			{
				return a_lhs == std::basic_string_view<value_type>(a_rhs ? a_rhs : EMPTY);
			}
			[[nodiscard]] inline friend bool operator!=(
				const BSFixedString& a_lhs,
				const_pointer a_rhs)
		...
		private:
			[[nodiscard]] static inline int strncmp(
				const char* a_lhs,
				const char* a_rhs,
				std::size_t a_length)
			{
			...
			{
				return _data ? reinterpret_cast<const BSStringPool::Entry*>(_data) - 1 : nullptr;
			}
	...
	{
	public:
		[[nodiscard]] inline std::uint32_t operator()(
			const detail::BSFixedString<CharT>& a_key) const noexcept
		{
